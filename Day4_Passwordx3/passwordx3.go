...
func has2Adiacent(code []int) bool {
	valid := 0
	if code[1] == code[0] {
		if code[1] != code[2] {
			valid++
	...
	if code[2] == code[1] {
		if code[2] != code[3] && code[1] != code[0] {
			valid++
		}
	}
	if code[3] == code[2] {
		if code[2] != code[1] && code[3] != code[4] {
			valid++
		}
	}
	if code[4] == code[3] {
		if code[5] != code[4] && code[3] != code[2] {
			valid++
		}
	}
	if code[5] == code[4] {
...
func main() {
	...
	for i := start; i <= end; i++ {
		code = numToArray(i)
		if isCrescent(code) && has2Adiacent(code) {
			numOfComb++
		}
	}
